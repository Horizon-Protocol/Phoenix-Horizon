...
contract('EtherCollateralsUSD', async accounts => {
	...
	const issuesUSDToAccount = async (issueAmount, receiver) => {
		...
		await hUSDSynth.transfer(receiver, issueAmount, {
			from: owner,
		});
	};
	...
	describe('when accessing the external views then', async () => {
		it('loanAmountFromCollateral should return 66.66 hUSD when opening a loan with 1 ETH @ 100', async () => {
		...
		it('loanAmountFromCollateral should return 10MM hUSD when opening a loan with 150000 ETH @ 100', async () => {
		...
		it('collateralAmountForLoan should return 1 ETH @ 100 required to open 66.66 hUSD', async () => {
		...
		it('collateralAmountForLoan should return 150000 ETH @ 100 required to open 10M hUSD', async () => {
	...
	describe('when opening a Loan', async () => {
		...
		describe('then revert when ', async () => {
			...
			it('attempting to issue more than the cap (issueLimit)', async () => {
				// limit hUSD supply cap to 100
				await etherCollateral.setIssueLimit(toUnit('100'), { from: owner });
				// 3 ETH will issue up to 200 hUSD
				await assert.revert(
			...
			it('attempting to issue more near the supply cap', async () => {
				// reduce the supply cap to 100 hUSD
		...
		describe('then create loan and', async () => {
			...
			it('add the loan issue amount minus minting fees to creators balance', async () => {
				...
				const sUSDBalance = await hUSDSynth.balanceOf(address1);
		...
		xdescribe('when creating multiple loans', () => {
			...
			beforeEach(async () => {
				// Issue hUSD to pay back interest
		...
		describe('when closing a Loan', async () => {
			...
			describe('check conditions', async () => {
				...
				it('when hUSD balance is less than loanAmount, then it reverts', async () => {
					// "Burn" some of accounts hUSD by sending to the owner
					await hUSDSynth.transfer(owner, toUnit('4'), { from: address1 });
			...
			describe('then it closes the loan and', async () => {
				...
				beforeEach(async () => {
					...
					feePoolBalanceBefore = await hUSDSynth.balanceOf(FEE_ADDRESS);
					// interestRatePerSec = await etherCollateral.interestPerSecond();
				...
				it('reduce hUSD totalSupply', async () => {
					assert.bnEqual(await etherCollateral.totalIssuedSynths(), ZERO_BN);
				});

				xit('increase the FeePool hUSD balance', async () => {
	...
	describe('when loanLiquidation is opened', async () => {
		...
		beforeEach(async () => {
			...
			// Chad opens hUSD loan to liquidate Alice
		...
		xit('then alice has a hUSD loan balance', async () => {
			assert.bnEqual(await hUSDSynth.balanceOf(alice), expectedsUSDLoanAmount);
		});
		...
		describe('when loanLiquidation is open', () => {
			...
			describe('when chad has some hUSD and alice has her hUSD still', () => {
				beforeEach(async () => {
					// Chad has already opened a hUSD loan
				});
				describe('and chad liquidates alices hUSD loan for her ETH', async () => {
					...
					it('then alice hUSD balance is still intact', async () => {
						assert.ok(await hUSDSynth.balanceOf(alice), expectedsUSDLoanAmount);
					});
					it('then chads hUSD balance is 0 as it was burnt to repay the loan', async () => {
						assert.ok(await hUSDSynth.balanceOf(chad), 0);
					});
			...
			describe('when bob has some hUSD', () => {
				beforeEach(async () => {
					await hUSDSynth.transfer(bob, await hUSDSynth.balanceOf(alice), { from: alice });
					await issuesUSDToAccount(toUnit('100'), bob);
				});
				describe('and bob liquidates alices hUSD loan for her ETH', async () => {
					let liquidateLoanTransaction;
					beforeEach(async () => {
						liquidateLoanTransaction = await etherCollateral.liquidateUnclosedLoan(alice, loanID, {
							from: bob,
					...
					it('then alice hUSD balance is 0 (because she transfered it to bob)', async () => {
						assert.ok(await hUSDSynth.balanceOf(alice), 0);
					});
					it('then bobs hUSD balance is 0 as it was burnt to repay the loan', async () => {
						assert.ok(await hUSDSynth.balanceOf(bob), 0);
					});
					it('then emits a LoanLiquidated event', async () => {
						assert.eventsEqual(
							liquidateLoanTransaction,
	...
	describe('when a loan is partially repaid', async () => {
		...
		it('should revert if the sender does not have enough hUSD to repay the amount requested', async () => {
