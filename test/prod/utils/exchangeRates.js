...
async function simulateExchangeRates({ network, deploymentPath }) {
	...
	currencyKeys = currencyKeys.filter(key => key !== toBytes32('hUSD'));
	const additionalKeys = ['HZN', 'BNB'].map(toBytes32); // The Depot uses the key "ETH" as opposed to "sETH" for its ether price
