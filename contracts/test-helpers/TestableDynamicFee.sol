...
contract TestableDynamicFee is Exchanger {
    ...
    function thresholdedAbsDeviationRatio(uint price, uint previousPrice, uint threshold) external view returns (uint) {
        return _thresholdedAbsDeviationRatio(price, previousPrice, threshold);
    }

    function dynamicFeeCalculation(uint[] calldata prices, uint threshold, uint weightDecay) external view returns (uint) {
